        ...
        <div class="main-content">
          ...
          <div class="left-panel">
            <AlgorithmPanel 
              ...
              :metrics-history="illuminationMetricsHistory"
...
import HeaderBar from "./guangzhao/header-bar.vue";
import AlgorithmPanel from "./guangzhao/algorithm-panel.vue";
import MonitorSection from "./guangzhao/monitor-section.vue";
    ...
    return {
      ...
      monitors: [
        ...
        },
        {
          ...
          timestamp: '14:23:48',
          environment: 'snow',
          videoUrl: require('@/assets/images/video2.mp4')
        }
      ],
  ...
  methods: {
    ...
    startPerformanceSimulation() {
      this.performanceTimer = setInterval(() => {
        ...
        // 更新光照自适应校准相关指标趋势
        this.updateIlluminationMetrics();
    ...
    startDataSimulation() {
      this.dataSimulationTimer = setInterval(() => {
        ...
        // 更新监控数据（简化为直接更新 monitors 数组）
        this.monitors = this.monitors.map(m => ({
          ...m,
          fps: Math.floor(Math.random() * 15) + 20,
          signalStatus: Math.random() > 0.95 ? 'warning' : 'normal',
          timestamp: new Date().toLocaleTimeString().slice(0, 8)
        }));

        // 随机更新对比数据（兼容现有 comparisons 结构）
        const processed = this.comparisons.find(c => c.id === 'processed');
        const detail = this.comparisons.find(c => c.id === 'detail');
        if (processed) {
          processed.processingTime = Math.floor(Math.random() * 30) + 15;
          processed.psnr = Number(((Math.random() * 8) + 22).toFixed(2));
          processed.ssim = Number((0.85 + Math.random() * 0.1).toFixed(3));
        }
        if (detail) {
          detail.processingTime = Math.floor(Math.random() * 15) + 20;
          detail.psnr = Number(((Math.random() * 4) + 27).toFixed(2));
          detail.ssim = Number((0.9 + Math.random() * 0.08).toFixed(3));
        }
        
      }, 1500);
    },

    // 更新光照评价指标历史（亮度、对比度、曝光补偿、校准误差、SSIM）
    updateIlluminationMetrics() {
      const ts = new Date().toLocaleTimeString().slice(3, 8); // mm:ss
      const mode = this.environmentMode;
      const algo = this.currentAlgorithm;

      // 环境与算法偏置，保证趋势合理
      const bias = {
        brightness: mode === 'fog' ? 10 : mode === 'snow' ? -5 : mode === 'rain' ? -2 : 0,
        contrast: mode === 'fog' ? -8 : mode === 'snow' ? 5 : 0,
        exposure: mode === 'snow' ? -6 : mode === 'rain' ? 3 : 0,
        error: mode === 'fog' ? 4 : mode === 'rain' ? 2 : mode === 'snow' ? 3 : 0,
        ssim: mode === 'fog' ? -0.02 : mode === 'rain' ? -0.01 : 0
      };

      const algoGain = algo === 'adaptive' ? 1.2 : algo === 'deep' ? 1.1 : 1.0;
      const rand = (base, jitter) => base + (Math.random() * jitter * 2 - jitter);

      const brightness = Math.max(0, Math.min(100, rand(60 + bias.brightness, 6) * algoGain));
      const contrast = Math.max(0, Math.min(100, rand(55 + bias.contrast, 6) * algoGain));
      const exposure = Math.max(-20, Math.min(20, rand(0 + bias.exposure, 3)));
      const calibrationError = Math.max(0, Math.min(15, rand(6 + bias.error, 2) / (algoGain))); // 越低越好
      const ssim = Math.max(0.6, Math.min(1.0, rand(0.88 + bias.ssim, 0.015)));

      const pushLimited = (arr, val) => {
        arr.push(val);
        if (arr.length > this.metricsMaxPoints) arr.shift();
      };

      pushLimited(this.illuminationMetricsHistory.timestamps, ts);
      pushLimited(this.illuminationMetricsHistory.brightness, Number(brightness.toFixed(1)));
      pushLimited(this.illuminationMetricsHistory.contrast, Number(contrast.toFixed(1)));
      pushLimited(this.illuminationMetricsHistory.exposure, Number(exposure.toFixed(1)));
      pushLimited(this.illuminationMetricsHistory.calibrationError, Number(calibrationError.toFixed(2)));
      pushLimited(this.illuminationMetricsHistory.ssim, Number(ssim.toFixed(3)));
    },
